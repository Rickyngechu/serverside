...
if (isset($_POST["submit"]) && isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
  ...
  $tourname = $conn->real_escape_string($tourname);
  ...
  if ($imageError == 0) {
    ...
    if (in_array(strtolower($imageExtension), $allowedExtensions)) {
      ...
      if (move_uploaded_file($imageTmpName, $uploadPath . $newImageName)) {
        ...
        $sqlData = "INSERT INTO tour (tourname, rating,people,difficulty,price,img,imgpath) VALUES (?, ?, ?, ?, ?)";

        // Prepare the statement
        $stmtData = $conn->prepare($sqlData);

        // Bind parameters for data
        $stmtData->bind_param("sssss", $tourname, $rating, $people,$difficulty,$price,$newImageName,);
